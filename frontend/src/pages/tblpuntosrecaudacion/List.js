...
const TblpuntosrecaudacionListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblpuntosrecaudacion/view/${data.punto_recaud_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblpuntosrecaudacion/edit/${data.punto_recaud_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.punto_recaud_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function PuntoRecaudIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblpuntosrecaudacion/view/${data.punto_recaud_id}`}> { data.punto_recaud_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblpuntosrecaudacionListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Puntos Recaudacion"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblpuntosrecaudacion/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="punto_recaud_id" header="Punto Recaud Id" body={PuntoRecaudIdTemplate}  ></Column>
                                    <Column  field="puntorecaud_nombre" header="Puntorecaud Nombre"   ></Column>
                                    <Column  field="puntorecaud_estado" header="Puntorecaud Estado"   ></Column>
...
TblpuntosrecaudacionListPage.defaultProps = {
	primaryKey: 'punto_recaud_id',
	pageName: 'tblpuntosrecaudacion',
	apiPath: 'tblpuntosrecaudacion/index',
	routeName: 'tblpuntosrecaudacionlist',
...
export default TblpuntosrecaudacionListPage;
