...
const TblpagosparquimetroListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblpagosparquimetro/view/${data.pago_parquimetro_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblpagosparquimetro/edit/${data.pago_parquimetro_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.pago_parquimetro_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function PagoParquimetroIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblpagosparquimetro/view/${data.pago_parquimetro_id}`}> { data.pago_parquimetro_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblpagosparquimetroListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Pagos Parquimetro"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblpagosparquimetro/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="pago_parquimetro_id" header="Pago Parquimetro Id" body={PagoParquimetroIdTemplate}  ></Column>
                                    <Column  field="parquimetro_id" header="Parquimetro Id"   ></Column>
...
TblpagosparquimetroListPage.defaultProps = {
	primaryKey: 'pago_parquimetro_id',
	pageName: 'tblpagosparquimetro',
	apiPath: 'tblpagosparquimetro/index',
	routeName: 'tblpagosparquimetrolist',
...
export default TblpagosparquimetroListPage;
