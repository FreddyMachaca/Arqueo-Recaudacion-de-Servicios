...
const TblvehiculosListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblvehiculos/view/${data.vehiculo_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblvehiculos/edit/${data.vehiculo_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.vehiculo_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function VehiculoIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblvehiculos/view/${data.vehiculo_id}`}> { data.vehiculo_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblvehiculosListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Vehiculos"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblvehiculos/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="vehiculo_id" header="Vehiculo Id" body={VehiculoIdTemplate}  ></Column>
                                    <Column  field="vehiculo_nombre" header="Vehiculo Nombre"   ></Column>
                                    <Column  field="vehiculo_celular" header="Vehiculo Celular"   ></Column>
                                    <Column  field="vehiculo_placa" header="Vehiculo Placa"   ></Column>
...
TblvehiculosListPage.defaultProps = {
	primaryKey: 'vehiculo_id',
	pageName: 'tblvehiculos',
	apiPath: 'tblvehiculos/index',
	routeName: 'tblvehiculoslist',
...
export default TblvehiculosListPage;
