...
const TbloperadoresListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tbloperadores/view/${data.operador_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tbloperadores/edit/${data.operador_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.operador_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function OperadorIdTemplate(data){
		if(data){
			return (
				<Link to={`/tbloperadores/view/${data.operador_id}`}> { data.operador_id }</Link>
			);
		}
	}
	...
	return (
<main id="TbloperadoresListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Operadores"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tbloperadores/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="operador_id" header="Operador Id" body={OperadorIdTemplate}  ></Column>
                                    <Column  field="per_id" header="Per Id"   ></Column>
                                    <Column  field="operador_estado" header="Operador Estado"   ></Column>
...
TbloperadoresListPage.defaultProps = {
	primaryKey: 'operador_id',
	pageName: 'tbloperadores',
	apiPath: 'tbloperadores/index',
	routeName: 'tbloperadoreslist',
...
export default TbloperadoresListPage;
