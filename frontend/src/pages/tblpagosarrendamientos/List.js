...
const TblpagosarrendamientosListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblpagosarrendamientos/view/${data.pago_arrendamiento_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblpagosarrendamientos/edit/${data.pago_arrendamiento_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.pago_arrendamiento_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function PagoArrendamientoIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblpagosarrendamientos/view/${data.pago_arrendamiento_id}`}> { data.pago_arrendamiento_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblpagosarrendamientosListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Pagos Arrendamientos"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblpagosarrendamientos/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="pago_arrendamiento_id" header="Pago Arrendamiento Id" body={PagoArrendamientoIdTemplate}  ></Column>
                                    <Column  field="arrendamiento_id" header="Arrendamiento Id"   ></Column>
                                    <Column  field="pago_monto" header="Pago Monto"   ></Column>
                                    <Column  field="pago_fecha" header="Pago Fecha"   ></Column>
                                    <Column  field="pago_estado" header="Pago Estado"   ></Column>
...
TblpagosarrendamientosListPage.defaultProps = {
	primaryKey: 'pago_arrendamiento_id',
	pageName: 'tblpagosarrendamientos',
	apiPath: 'tblpagosarrendamientos/index',
	routeName: 'tblpagosarrendamientoslist',
...
export default TblpagosarrendamientosListPage;
